...
import { QuestionMarkCircleIcon, MoonIcon } from "@heroicons/react/24/outline";
import DashboardHomeIcon from "@/components/icons/dashboard-home.svg";
import DashboardAnalyticsIcon from "@/components/icons/bar-line-chart.svg";
import DashboardWalletIcon from "@/components/icons/wallet-02.svg";
import DashboardAccountsIcon from "@/components/icons/user-03.svg";
import DashboardSettingsIcon from "@/components/icons/settings-01.svg";
import DashboardArrowDown from "@/components/icons/arrow-down.svg";
  ...
  <Link
    ...
    className={`flex items-center gap-3 px-4 py-3 cursor-pointer rounded-lg ${
      active
        ...
        : "text-gray-200 hover:bg-purple-100/10"
    }`}
  >
...
function DashboardSidebar() {
  ...
  return (
    <div className="flex flex-col h-full w-full px-4 py-4">
      ...
      {/* Main Navigation */}
      <nav className="flex-1 space-y-3 mt-5">
        <NavItem
          icon={
            <DashboardHomeIcon
              className={
                pathname === "/dashboard" ? "text-purple-900" : " text-white"
              }
            />
          }
          label="Dashboard"
        ...
        <NavItem
          icon={
            <DashboardAnalyticsIcon
              className={pathname === "/analytics" ? "text-purple-900" : ""}
            />
          }
          label="Analytics"
        ...
        <NavItem
          icon={
            <DashboardAccountsIcon
              className={
                pathname === "/accounts" ? "text-purple-900" : "text-white"
              }
            />
          }
          label="Accounts"
        ...
        <NavItem
          icon={
            <DashboardSettingsIcon
              className={
                pathname === "/settings" ? "text-purple-900" : "text-white"
              }
            />
          }
          label="Settings"
          ...
          <NavItem
            icon={
              <QuestionMarkCircleIcon
                className={pathname === "/help-centre" ? "text-purple-900" : ""}
              />
            }
            label="Help centre"
            active={pathname === "/help-centre"}
            href="/help-centre"
          />
          <NavItem icon={<MoonIcon />} label="Dark theme" href="/dark-theme" />
        </div>
      </nav>

      {/* User Profile */}
      <div className="mt-auto pt-4 ">
        <div className="flex items-center gap-3 px-4 py-2 justify-between">
          <div className="flex items-center gap-3">
            <Image src="/lady.svg" alt="User Avatar" width={32} height={32} />
            <div>
              <h4 className="text-white font-medium">Ali Riaz</h4>
